...
13. 教师端任务资源编辑区统一采用“左侧源码 / 资源、右侧预览”的并排布局；网页任务与数据提交任务的“学生提交页 / 数据可视化页”都遵循同一套分栏编辑模式。
14. 学案编辑器需要具备本地草稿自动保存与离开未保存提醒，确保教师在切换学案、切换路由或刷新页面时不会丢失正文、任务说明与任务源码修改。
15. 教师端 AI 生成任务页时，需同时支持“提示词模板 + 自定义补充要求”；网页任务、数据提交页、数据提交可视化页都可分别选择适配模板。
16. 预览失败时，教师端必须展示明显的错误反馈面板，支持重试、展开完整错误详情 / 堆栈，并提供一键复制错误详情，方便教师自查或继续交给 AI 修正。
17. 学案编辑器前端实现需把“编辑器状态 / 任务行操作 / 任务模板库 / 任务预览与资源上传”等重状态逻辑拆到可复用 composable 中，避免继续把复杂状态堆积在单一页面组件内。
...
## 21. 2026-04-06 教师学案编辑器写实同步

本次针对教师学案页补充写实以下当前状态：

1. 学案编辑器体验
   - 学案正文与任务区仍采用双 Tab，但教师在选中学案后可直接进入同页编辑，不必再先进入二级弹层。
   - 编辑中的学案会自动写入本地草稿；离开当前页、切换路由或刷新前，会触发“未保存提醒”，避免正文与任务源码丢失。
2. 网页任务 / 数据提交任务编辑器
   - 网页任务、学生提交页、数据可视化页都已统一为左右分栏：左侧编辑源码与资源，右侧即时预览。
   - 数据提交任务切换类型后会优先预生成真实任务编号，并直接展示正式的“提交接口 / 读取接口”绝对路径，方便教师或 AI 直接使用。
   - 未保存为正式资源的 HTML 也支持 `srcdoc` 即时预览；保存后继续切换到真实任务资源预览。
3. AI 生成与模板库
   - 任务 HTML 生成已支持“提示词模板 + 自定义补充要求”，并按网页任务、提交页、可视化页分别提供模板。
   - 自定义任务模板库已支持另存为、覆盖保存、分组、搜索、置顶、最近使用、拖拽排序、跨分组拖拽、批量改分组、按分组整组选择、批量置顶与批量取消置顶。
4. 预览错误反馈
   - 教师创建网页后，预览区会在加载失败时显示明确的错误反馈面板。
   - 面板支持重试预览、展开完整详情 / 堆栈，以及一键复制错误详情，便于继续排障。
5. 前端结构整理
   - 学案编辑器状态已拆到 `useLessonPlanEditorState`。
